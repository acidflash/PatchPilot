...
# PatchPilot agent self-update upgrade v3
# Fixes: "Could not find apt_clean block in execute_job"
#
# Run:
#   cd patchpilot
#   bash patchpilot-add-agent-selfupdate-v3.sh
...
TS="$(date +%Y%m%d-%H%M%S)"
BACKUP_DIR="_backup_selfupdate_v3_${TS}"
...
if not m:
    raise SystemExit("Could not find ALLOWED_ACTIONS in backend/app/main.py")
...
# Replace entire checkin function. This avoids fragile matching.
...
new_checkin = r'''@app.post("/api/v1/agent/checkin")
def checkin(
    payload: dict,
    request: Request,
...
echo "[+] Patching agent by rewriting self-update + execute_job + run_once loop"
...
# Insert or replace self_update_agent
self_update_func = r'''def self_update_agent(update_url, expected_sha256=None):
    if not update_url:
...
if "def self_update_agent(" not in txt:
    ...
    txt = txt.replace(marker, self_update_func + "\n" + marker, 1)

# Replace entire execute_job function
start = txt.find("def execute_job(job):")
if start == -1:
    raise SystemExit("Could not find execute_job function")

m_next = re.search(r'\ndef\s+\w+\(', txt[start + 1:])
if not m_next:
    raise SystemExit("Could not find end of execute_job function")
end = start + 1 + m_next.start()

new_execute = r'''def execute_job(job):
...
'''
txt = txt[:start] + new_execute + txt[end+1:]

# Replace entire run_once function to support agent update metadata
start = txt.find("def run_once():")
if start == -1:
    raise SystemExit("Could not find run_once function")

m_next = re.search(r'\nif __name__ == "__main__":', txt[start + 1:])
if not m_next:
    raise SystemExit("Could not find end of run_once function")
end = start + 1 + m_next.start()

new_run_once = r'''def run_once():
    cfg = load_config()
...
'''
txt = txt[:start] + new_run_once + txt[end+1:]
...
echo
echo "[+] Agent self-update v3 patch complete."
echo
...
echo "  docker compose logs -n 100 backend"
echo
